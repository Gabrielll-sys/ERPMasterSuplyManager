...
const updateAtividade  = async(atividade: IAtividadeRd, status: string, observacoes: string,descricao:string)=>{
    ...
    const generatePlanilha = async ()=>{
        ...
        */
        ...
        ws.getCell(`A${atividadesInRd.length+4}`).value= `Status Relátorio: ${relatorioDiario.isFinished ? `Relatório Conclúido ${dayjs(relatorioDiario?.dataFechamento).format(`DD/MM/YYYY [as] HH:mm:ss`)} ` :"Em Análise"} `
