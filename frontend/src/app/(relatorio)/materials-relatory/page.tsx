...
export default function MaterialRelatorio(){
...
const filtro  = {
  ...
  setMateriaisFiltro(materiaisFiltrados)
   ...
   <div className='flex flex-row justify-center mt-10'>
     <Button className="text-white bg-master_black p-4 font-bold ml-5 text-lg" onClick={()=>gerarRelatorio()}>
     Filtrar
     </Button>
   ...
   <div className='mt-16 flex '  ref={componentRef}>
      ...
      <Table  hoverable striped className="w-[100%] ">
        ...
        { materiaisFiltros.length>=1 && materiaisFiltros.map((row:any) => (
          ...
          L222: <Table.Cell className="text-center text-black hover:underline" onClick={()=>route.push(`/atualizar-inventario/${row.material.id}`)}>{row.saldoFinal==null?"Não registrado":row.saldoFinal +" "+row.material.unidade}</Table.Cell>
