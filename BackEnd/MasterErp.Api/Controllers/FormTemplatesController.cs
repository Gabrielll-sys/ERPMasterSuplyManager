    ...
    public class FormTemplatesController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateFormTemplate([FromBody] FormTemplateCreateDto createDto)
        {
            ...
            try
            {
                ...
                var newTemplateDetail = await _formTemplateService.CreateTemplateAsync(createDto, createdByUserId);

                if (newTemplateDetail == null) // O serviço pode retornar null se algo der errado e não for uma exceção
                {
                    _logger.LogError("Serviço CreateTemplateAsync retornou null para o template '{TemplateName}' pelo usuário ID: {UserId}", createDto.Name, createdByUserId);
